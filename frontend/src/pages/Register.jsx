...
import { register } from '../services/authService'; // Named Import chuẩn
...
const Register = () => {
  const [formData, setFormData] = useState({
    ...
    password: '',
    phone: '',
    role_id: 1, // Mặc định là khách hàng (USER)
  });
  const navigate = useNavigate();

  const handleSubmit = async (e) => {
    e.preventDefault();
    console.log("Dữ liệu gửi đi:", formData); // Debug để kiểm tra trước khi gửi
    
    try {
      // SỬA TẠI ĐÂY: Gọi trực tiếp hàm register và truyền cả object formData
      ...
      // Hiển thị lỗi từ backend (ví dụ: Email đã tồn tại) hoặc lỗi mặc định
  ...
  <div className="d-flex justify-content-center align-items-center" 
    ...
    <div className="card border-0" 
      ...
      <div className="text-center mb-5">
        {/* Thêm một icon nhỏ phía trên để tăng độ nhận diện thương hiệu */}
        ...
        <p className="text-muted" style={{ fontSize: '0.9rem' }}>Bắt đầu trải nghiệm đặt sân chuyên nghiệp cùng S.BOOK</p>
      </div>

      <form onSubmit={handleSubmit}>
        {/* Style chung cho Input Group để trông hiện đại hơn */}
        {[
        ...
        <button type="submit" className="btn btn-book-now w-100 py-3 shadow-sm mt-2" 
