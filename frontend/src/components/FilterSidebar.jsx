...
const FilterSidebar = ({ onResults }) => {
  ...
  useEffect(() => {
    const fetchFilteredFields = async () => {
      try {
        ...
        const response = await axios.get('http://localhost:5000/api/fields/search', { params });
    ...
    if (value === 'gan_nhat') {
      if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(
          ...
          (error) => {
            alert('Không thể lấy được vị trí của bạn. Vui lòng kiểm tra quyền truy cập vị trí.');
  ...
  const clearFilters = () => {
    setFilters({keyword: '', type: '', minPrice: 0, maxPrice: 500000, minRating: 0, sortBy: '', userLat: null, userLng: null});
  };

  return (
    <div className="p-4 bg-white border rounded-4 shadow-sm h-100">
      <h4 className="fw-bold mb-4 text-dark"><i className="bi bi-funnel-fill text-success me-2"></i> Lọc Tìm Kiếm</h4>
      ...
      <div className="mb-4">
        <label className="form-label fw-semibold text-secondary d-flex justify-content-between">
          ...
          <span className="text-success fw-bold">{Number(filters.maxPrice).toLocaleString('vi-VN')} đ/h</span>
        </label>
        <input 
          type="range" 
          name="maxPrice"
        ...
        <div className="d-flex gap-2">
          {[1, 2, 3, 4, 5].map((star) => (
            <i
              ...
              className={`bi bi-star-fill fs-4 cursor-pointer ${filters.minRating >= star ? 'text-warning' : 'text-light'}`}
