...
// 13. Theo dõi dòng tiền của User (cho Admin và Owner)
exports.getUserCashFlow = async (req, res) => {
  try {
    ...
    if (currentUser.role !== "admin") {
      // Nếu không phải admin, kiểm tra xem có phải owner của sân mà user này đã đặt không
      ...
      if (userBookings.length === 0) {
        return res.status(403).json({
          success: false,
          message: "Bạn không có quyền xem dòng tiền của user này",
        });
      }
    }
    ...
    for (const booking of bookings) {
      // Giao dịch đặt sân (tiền ra)
      transactions.push({
        id: `booking_${booking.id}`,
        type: "booking",
        description: `Đặt sân ${booking.field?.name || "N/A"} tại ${booking.stadium?.name || "N/A"}`,
      ...
      // Giao dịch thanh toán (tiền vào cho hệ thống)
      if (
        ...
        transactions.push({
          ...
          type: "payment",
          description: `Thanh toán cho booking #${booking.id}`,
          amount: parseFloat(booking.amount_paid || 0),
      ...
      // Giao dịch hoàn tiền (tiền ra từ hệ thống)
      if (booking.status === "refunded") {
        transactions.push({
          ...
          amount: -parseFloat(booking.total_price || 0),
          date: booking.refunded_at || booking.updatedAt,
          status: "refunded",
    ...
    console.error("Lỗi lấy dòng tiền user:", error);
    res.status(500).json({
      success: false,
